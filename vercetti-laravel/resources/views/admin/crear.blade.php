...
<head>
  <meta charset="UTF-8">
  <title>Nueva Propiedad | Vercetti Admin</title>
...
<div class="form-container">
  <h2>NUEVA <span>PROPIEDAD</span></h2>
  ...
  <form method="POST" action="{{ route('admin.store') }}">
    ...
    <div class="form-grid">
      ...
      <div class="form-group">
        ...
        <select name="tipo_inmueble">
          ...
          @foreach(['Casa','Edificio','Local','Garage'] as $tipo)
            <option value="{{ $tipo }}" {{ old('tipo_inmueble') === $tipo ? 'selected' : '' }}>{{ $tipo }}</option>
          @endforeach
        </select>
      ...
      <div class="form-group">
        ...
        <input type="number" name="precio" min="0" value="{{ old('precio') }}">
        @error('precio') <span class="error-msg">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label>Metros cuadrados <span>*</span></label>
        <input type="number" name="metros_cuadrados" min="0" value="{{ old('metros_cuadrados') }}">
        @error('metros_cuadrados') <span class="error-msg">{{ $message }}</span> @enderror
      </div>

      <div class="form-group">
        <label>Habitaciones</label>
        <input type="number" name="num_habitaciones" min="0" value="{{ old('num_habitaciones') }}">
      </div>

      <div class="form-group">
        <label>Fecha de construcción <span>*</span></label>
        <input type="date" name="fecha_construccion" value="{{ old('fecha_construccion') }}">
      ...
      <div class="form-group full">
        ...
        <input type="text" name="imagen" placeholder="imagenes/propiedadesVC/nombre.jpg" value="{{ old('imagen') }}">
      </div>
      ...
      <div class="form-group full">
        <div class="form-check">
          <input type="checkbox" name="amueblada" id="amueblada" value="1" {{ old('amueblada') ? 'checked' : '' }}>
    ...
    <div class="form-actions">
      <button type="submit" class="btn btn-gold">GUARDAR PROPIEDAD</button>
